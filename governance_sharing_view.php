...
$stmt = $conn->prepare("SELECT g.*, u.email AS uploader_email FROM governance_sharing_uploads g JOIN users u ON g.employee_id = u.id WHERE g.id = ?");
...
$filePath = __DIR__ . '/uploads/governance_sharing/' . $file['filename'];
...
<body>
  ...
  <div class="page-wrapper">
    ...
    <div class="header">
        ...
        <div>
            <a href="uploads/governance_sharing/<?= h($file['filename']) ?>" class="btn btn-light btn-sm me-2" download>
                <i data-lucide="download" class="me-1"></i> Download
            </a>
            <a href="governance_sharing.php" class="btn btn-outline-light btn-sm">
                <i data-lucide="arrow-left" class="me-1"></i> Back
            </a>
        </div>
    </div>
    <iframe src="uploads/governance_sharing/<?= h($file['filename']) ?>" class="pdf-viewer"></iframe>
      </div>
    ...
    <div class="container">
        <div class="header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="uploads/governance_sharing/<?= h($file['filename']) ?>" class="btn btn-primary btn-sm me-2" download>
                        <i data-lucide="download" class="me-1"></i> Download
                    </a>
                    <a href="governance_sharing.php" class="btn btn-outline-secondary btn-sm">
                        <i data-lucide="arrow-left" class="me-1"></i> Back
                    </a>
                </div>
            </div>
        ...
        <div class="image-container">
            <img src="uploads/governance_sharing/<?= h($file['filename']) ?>" alt="<?= h($file['original_name']) ?>">
        </div>
    </div>
      </div>
  <?php include PGS_TEMPLATES . '/footer.php'; ?>
