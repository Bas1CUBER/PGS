  ...
  try {
    if ($action === 'save_table1') {
      ...
      if (in_array($field, $allowedNumeric, true)) {
        ...
        $stmt = $pdo->prepare("UPDATE training_pct_personnel SET {$field} = :v WHERE id = :id");
        $stmt->execute([':v'=>$val, ':id'=>$id]);
        // Notify all users
        $userInfo = getUserInfo($userId);
        $userIdent = formatUserIdentifier($userInfo);
        $roleLabel = ucfirst($role);
        $notifMsg = $roleLabel . " " . $userIdent . " updated " . strtoupper($field) . " in Percentage Trained personnel";
        notifyAdmins('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
        notifyFocals('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
        ...
        $userInfo = getUserInfo($userId);
        $userIdent = formatUserIdentifier($userInfo);
        $notifMsg = "Admin " . $userIdent . " updated " . $field . " in Percentage Trained personnel";
        notifyAdmins('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
        notifyFocals('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
        echo json_encode(['ok'=>true]); exit;
      } else {
    ...
    if ($action === 'add_table1') {
      ...
      $stmt = $pdo->prepare("INSERT INTO training_pct_personnel (section, personnel, is_head, created_by) VALUES (:s,:p,:h,:cb)");
      $stmt->execute([':s'=>$section, ':p'=>$personnel, ':h'=>$is_head, ':cb'=>$userId ?: null]);
      $newId = (int)$pdo->lastInsertId();
      // Notify all users
      $userInfo = getUserInfo($userId);
      $userIdent = formatUserIdentifier($userInfo);
      $notifMsg = "Admin " . $userIdent . " added new personnel: " . $personnel . " in Percentage Trained";
      notifyAdmins('upload', 'Percentage Trained Updated', $notifMsg, $newId, 'percentage_trained');
      notifyFocals('upload', 'Percentage Trained Updated', $notifMsg, $newId, 'percentage_trained');
      echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'add_table2') {
      ...
      $stmt = $pdo->prepare("INSERT INTO training_pct_events (serial_no, title, training_type, participants, date_label, year, locked, created_by) VALUES (:sn,:t,:tt,:p,:d,:y,0,:cb)");
      ...
      $userIdent = formatUserIdentifier($userInfo);
      $roleLabel = ucfirst($role);
      $notifMsg = $roleLabel . " " . $userIdent . " added training event: " . $name . " in Percentage Trained";
      notifyAdmins('upload', 'Percentage Trained Updated', $notifMsg, $newId, 'percentage_trained');
      notifyFocals('upload', 'Percentage Trained Updated', $notifMsg, $newId, 'percentage_trained');
      echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'edit_table2') {
      ...
      preg_match('/(20\\d{2})/', $date_label, $m);
      $year = isset($m[1]) ? (int)$m[1] : null;
      $stmt = $pdo->prepare("UPDATE training_pct_events SET serial_no = :sn, title = :t, training_type = :t, participants = :p, date_label = :d, year = :y WHERE id = :id");
      $stmt->execute([':sn'=>$serial_no, ':t'=>$name ?: null, ':p'=>$participants ?: null, ':d'=>$date_label ?: null, ':y'=>$year, ':id'=>$id]);
      // Notify all users
      $userInfo = getUserInfo($userId);
      $userIdent = formatUserIdentifier($userInfo);
      $roleLabel = ucfirst($role);
      $notifMsg = $roleLabel . " " . $userIdent . " edited training event: " . $name . " in Percentage Trained";
      notifyAdmins('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
      notifyFocals('edit', 'Percentage Trained Updated', $notifMsg, $id, 'percentage_trained');
      echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'save_table2') {
      ...
      if ($field === 'training_type' || $field === 'title') {
        $stmt = $pdo->prepare("UPDATE training_pct_events SET title = :v, training_type = :v WHERE id = :id");
        $stmt->execute([':v'=>$val, ':id'=>$id]);
      } else {
        $stmt = $pdo->prepare("UPDATE training_pct_events SET {$field} = :v WHERE id = :id");
        $stmt->execute([':v'=>$val, ':id'=>$id]);
      }
      if ($field === 'date_label') {
        preg_match('/(20\\d{2})/', $val, $m);
        $year = isset($m[1]) ? (int)$m[1] : null;
        $pdo->prepare("UPDATE training_pct_events SET year = :y WHERE id = :id")->execute([':y'=>$year, ':id'=>$id]);
      }
      echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'set_lock_table2') {
      ...
      $pdo->prepare("UPDATE training_pct_events SET locked = :l WHERE id = :id")->execute([':l'=>$locked, ':id'=>$id]);
      echo json_encode(['ok'=>true]); exit;
    }
    if ($action === 'delete_table2') {
      if ($role !== 'admin') throw new Exception('Admin only');
      $id = (int)($_POST['id'] ?? 0);
      $pdo->prepare("DELETE FROM training_pct_events WHERE id = :id")->execute([':id'=>$id]);
      echo json_encode(['ok'=>true]); exit;
    }
    ...
    if ($action === 'focal_delete_table1_person') {
      ...
      $pdo->prepare("DELETE FROM training_pct_personnel WHERE id = :id")->execute([':id'=>$id]);
      echo json_encode(['ok'=>true]); exit;
    }
...
// Setup tables
$pdo->exec("
  CREATE TABLE IF NOT EXISTS training_pct_personnel (
...
");
// Ensure section column exists in older deployments
try {
  $chkSection = $pdo->query("SHOW COLUMNS FROM training_pct_personnel LIKE 'section'");
  if ($chkSection->rowCount() === 0) {
    $pdo->exec("ALTER TABLE training_pct_personnel ADD COLUMN section VARCHAR(80) NOT NULL DEFAULT '' AFTER id");
  }
  $chkY2028 = $pdo->query("SHOW COLUMNS FROM training_pct_personnel LIKE 'y2028'");
  if ($chkY2028->rowCount() === 0) {
    $pdo->exec("ALTER TABLE training_pct_personnel ADD COLUMN y2028 INT DEFAULT NULL AFTER y2027");
  }
} catch (Throwable $e) { /* ignore */ }
$pdo->exec("
  CREATE TABLE IF NOT EXISTS training_pct_events (
    id INT AUTO_INCREMENT PRIMARY KEY,
...
try {
  $chk = $pdo->query("SHOW COLUMNS FROM training_pct_events LIKE 'training_type'");
  if ($chk->rowCount() === 0) {
    $pdo->exec("ALTER TABLE training_pct_events ADD COLUMN training_type VARCHAR(255) DEFAULT NULL AFTER title");
  } else {
    $pdo->exec("ALTER TABLE training_pct_events MODIFY COLUMN training_type VARCHAR(255) DEFAULT NULL");
  }
  $chk2 = $pdo->query("SHOW COLUMNS FROM training_pct_events LIKE 'serial_no'");
  if ($chk2->rowCount() === 0) {
    $pdo->exec("ALTER TABLE training_pct_events ADD COLUMN serial_no INT DEFAULT NULL AFTER id");
  }
  // Make key columns nullable
  $pdo->exec("ALTER TABLE training_pct_events 
...
$count = (int)$pdo->query("SELECT COUNT(*) FROM training_pct_personnel")->fetchColumn();
if ($count === 0) {
  $prefill = [
  ...
  $ins = $pdo->prepare("INSERT IGNORE INTO training_pct_personnel (section, personnel, is_head, created_by) VALUES (:s,:p,:h,:cb)");
...
// Fetch rows
$personnelRows = $pdo->query("SELECT * FROM training_pct_personnel ORDER BY section, is_head DESC, personnel")->fetchAll(PDO::FETCH_ASSOC);
$eventsRows = $pdo->query("SELECT * FROM training_pct_events ORDER BY id ASC")->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container" pt-110>
  <div class="header-wrap">
    ...
    <div class="header-title">
      <h4>Governance Scorecard: Percentage of SFLU TRC Personnel with direct patient care are trained by Certified Trainers</h4>
  ...
  <div class="card mb-4">
    <div class="card-header">
      <div class="d-flex align-items-center gap-2">
        <span class="section-title flex-grow-1">Table 1. Number of SFLU TRC Personnel with Direct Patient Care Trained by Certified Trainers</span>
  ...
  <div class="card mb-4">
    ...
    <div class="card-body">
      <?php if (in_array($role, ['admin','employee','focal'], true)): ?>
...
<?php $pgsPage = ['role' => $role, 'years' => $years, 'trainingsByYear' => $trainingsByYear]; ?><script>window.PGS.page = <?= json_encode($pgsPage) ?>;</script><script src="<?= asset('js/pages/training_roadmap_percentage_trained_1.js') ?>"></script>
      ...
      <div class="modal-body">
        <form id="formEdit" class="row g-3">
          ...
          <div class="col-12 col-lg-10">
            ...
            <textarea class="form-control form-control-sm" name="name" rows="3" maxlength="255"></textarea>
          </div>
          <div class="col-12">
