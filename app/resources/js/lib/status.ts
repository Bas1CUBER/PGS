/**
 * Shared status vocabularies — single source of truth for workflow states.
 *
 * Server truth lives in WorkflowRegistry (deliverables) and
 * UploadModuleRegistry (modules). This file mirrors those vocabularies for
 * frontend display logic so pages do not re-declare local maps.
 */

export const DeliverableStatuses = ['Not Yet Started', 'Ongoing', 'Accomplished'] as const;
export type DeliverableStatus = (typeof DeliverableStatuses)[number];

export const ReviewStatuses = {
    Pending: 'Pending',
    InProgress: 'In Progress',
    Approved: 'Approved',
    Returned: 'Returned',
} as const;

export const CommPlanStatuses = ['Not Accomplished/Started', 'Ongoing', 'Completed'] as const;

export const StatusStyleMap: Record<string, string> = {
    Approved: 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200',
    Accomplished: 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200',
    Completed: 'bg-green-100 text-green-800 dark:bg-green-900/40 dark:text-green-200',
    Pending: 'bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-200',
    Returned: 'bg-red-100 text-red-800 dark:bg-red-900/40 dark:text-red-200',
    'In Progress': 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200',
    Ongoing: 'bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-200',
    'Not Yet Started': 'bg-slate-100 text-slate-800 dark:bg-slate-900/40 dark:text-slate-200',
    'Not Accomplished/Started': 'bg-slate-100 text-slate-800 dark:bg-slate-900/40 dark:text-slate-200',
};

export function statusClass(status: string | null | undefined): string {
    if (!status) return 'bg-slate-100 text-slate-800';
    return StatusStyleMap[status] ?? 'bg-slate-100 text-slate-800';
}
