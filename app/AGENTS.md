...
- Background work needs `scripts\register-scheduler-tasks.ps1` (no admin): registers "PgsScheduler" (every minute → `schedule:run`) and "PgsQueueWorker" (every 5 min, drains the database queue) as hidden-window PowerShell tasks for the logged-in user — same pattern as `..\hris\scripts`. Without them, deadlines, nightly backups (`01:30`), outbox pruning, and queued backup jobs silently stop.
