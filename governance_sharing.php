...
$tableExists = false;
try {
    $res = $conn->query("SHOW TABLES LIKE 'governance_sharing_uploads'");
...
if (!$tableExists) {
    $conn->query("
        CREATE TABLE IF NOT EXISTS governance_sharing_uploads (
...
try {
    $colCheck = $conn->query("SELECT COUNT(*) AS c FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'governance_sharing_uploads' AND COLUMN_NAME = 'status_updated_at'");
    $colRow = $colCheck ? $colCheck->fetch_assoc() : null;
    if ($colRow && (int)$colRow['c'] === 0) {
        $conn->query("ALTER TABLE governance_sharing_uploads ADD COLUMN status_updated_at TIMESTAMP NULL DEFAULT NULL AFTER status");
    }
} catch (Throwable $e) {}
$uploadDir = __DIR__ . '/uploads/governance_sharing/';
if (!is_dir($uploadDir)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'upload_pdf' || $action === 'upload_image') {
        if (!isset($_FILES['file']) || !is_uploaded_file($_FILES['file']['tmp_name'])) {
            header("Location: governance_sharing.php");
            exit();
        }
        $file = $_FILES['file'];
        ...
        $unique = 'gov_sharing_' . $userId . '_' . time() . '.' . $ext;
        $path = $uploadDir . $unique;
        if (move_uploaded_file($file['tmp_name'], $path)) {
            $docType = $isPdf ? 'PDF' : 'Image';
            $stmt = $conn->prepare("INSERT INTO governance_sharing_uploads (title, description, employee_id, filename, original_name, file_size, mime_type, doc_type) VALUES (?,?,?,?,?,?,?,?)");
            $stmt->bind_param(
                "ssississ",
                $_POST['title'],
                $_POST['description'],
            ...
            notifyAdmins('upload', 'New File Uploaded', $notifMsg, $uploadId, 'governance_sharing');
            notifyFocals('upload', 'New File Uploaded', $notifMsg, $uploadId, 'governance_sharing');
        }
        header("Location: governance_sharing.php");
        exit();
    }
    if ($role === 'admin' && $action === 'update_status') {
        ...
        if ($id > 0 && in_array($status, ['In Progress','Approved','Returned'], true)) {
            $stmt = $conn->prepare("SELECT g.employee_id, g.title, u.email AS uploader_email FROM governance_sharing_uploads g JOIN users u ON g.employee_id = u.id WHERE g.id = ?");
            $stmt->bind_param("i", $id);
            $stmt->execute();
            $uploadInfo = $stmt->get_result()->fetch_assoc();
            
            $stmt = $conn->prepare("UPDATE governance_sharing_uploads SET status=?, status_updated_at=NOW() WHERE id=?");
            ...
            if ($uploadInfo) {
                ...
                if ($status === 'Approved') {
                    ...
                    notifyFocals('approved', 'File Approved', 'Admin has approved the file uploaded by ' . $uploaderId . ': "' . $title . '"', $id, 'governance_sharing');
                    ...
                    notifyFocals('returned', 'File Returned', 'Admin has returned the file uploaded by ' . $uploaderId . ': "' . $title . '"', $id, 'governance_sharing');
                }
            }
        }
        header("Location: governance_sharing.php");
...
$uploads = [];
$q = $conn->query("SELECT g.*, u.email AS uploader_email FROM governance_sharing_uploads g JOIN users u ON g.employee_id = u.id ORDER BY g.uploaded_at DESC");
...
<script src="<?= asset('js/pages/governance_sharing_1.js') ?>"></script>
