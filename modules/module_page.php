...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
  ...
  try {
    if ($action === 'delete_row') {
      ...
      if (!$category) { echo json_encode(['status'=>'error','message'=>'Invalid category']); exit; }
    ...
    if ($action === 'save_progress') {
      if (!in_array($role, ['employee','focal'], true)) { echo json_encode(['ok'=>false,'msg'=>'Read-only']); exit; }
      $category = trim($_POST['category'] ?? '');
    ...
    if ($action === 'add_row_item') {
      ...
      if (!$category || !$year || !$desc) { echo json_encode(['ok'=>false,'msg'=>'Missing fields']); exit; }
    ...
    if ($action === 'delete_progress_cell') {
      if ($role !== 'admin') { echo json_encode(['ok'=>false,'msg'=>'Unauthorized']); exit; }
      $category = trim($_POST['category'] ?? '');
      $year = (int)($_POST['year'] ?? 0);
      $month = (int)($_POST['month'] ?? 0);
      if (!$category || !$year || $month < 1 || $month > 12) { echo json_encode(['ok'=>false,'msg'=>'Invalid input']); exit; }
...
$categories = [];
while ($row = $categoriesResult->fetch_assoc()) { $categories[] = $row['category']; }
...
<div class="page-wrapper container my-5">
  <div class="card shadow-sm mb-5">
    <div class="card-body">
      <div class="d-flex align-items-center mb-3">
        <img src="<?= $baseUrl ?>/assets/img/<?= $mod['image'] ?>" alt="<?= $mod['title'] ?>" style="max-width: 100px; height: auto;" class="me-3">
        <h1 class="card-title h3 text-secondary fw-bold mb-0"><?= $mod['title'] ?></h1>
      </div>
      <p class="text-muted" style="text-align: justify;"><?= $mod['description'] ?></p>
    </div>
  </div>
    ...
    <div class="card-body">
      ...
      <div class="d-flex gap-2 flex-wrap">
        <?php foreach ($mod['roadmaps'] as $rp): ?>
          <a href="<?= $rp ?>" class="btn btn-outline-primary btn-sm"><?= pathinfo($rp, PATHINFO_FILENAME) ?></a>
        <?php endforeach; ?>
      </div>
    </div>
  </div>
        ...
        <table class="table table-bordered align-middle">
          ...
          <tbody>
            ...
            try {
              ...
              while ($q && ($r = $q->fetch_assoc())) { $pend[] = $r; }
            } catch (Throwable $e) {}
            if (empty($pend)): ?>
            <tr><td colspan="7" class="text-center text-muted">No pending changes</td></tr>
            <?php else: foreach ($pend as $p): ?>
            <tr>
              <td><?= htmlspecialchars($p['submitter_email'] ?? (string)$p['submitted_by']) ?></td>
              <td><?= htmlspecialchars($p['category']) ?></td>
              <td><?= (int)$p['year'] ?></td>
              <td><?= $p['change_type'] === 'add_row' ? 'Add Row' : 'Progress Update' ?></td>
              <td><?php if ($p['change_type'] === 'add_row'): ?><?= htmlspecialchars($p['description']) ?><?php else: ?><div><strong>Month:</strong> <?= (int)$p['month'] ?></div><div><strong>Status:</strong> <?= htmlspecialchars($p['status']) ?></div><div><strong>Remarks:</strong> <?= htmlspecialchars($p['remarks']) ?></div><div><strong>Description:</strong> <?= htmlspecialchars($p['description']) ?></div><?php endif; ?></td>
              <td><?= htmlspecialchars(date('Y-m-d H:i:s', strtotime($p['submitted_at']))) ?></td>
              <td>
  ...
  <div class="dashboard-container">
    ...
    <div class="dashboard-card year-selector-card">
      ...
      <select id="dashboardYear" class="form-select"><?php foreach ($years as $y): ?><option value="<?= $y ?>"><?= $y ?></option><?php endforeach; ?></select>
    ...
    <div class="card-body">
      <div class="d-flex align-items-center justify-content-between mb-3">
        ...
        <div class="d-flex align-items-center gap-2">
          ...
          <select id="progressYear" class="form-select form-select-sm" style="width:140px"><?php foreach ($years as $y): ?><option value="<?= $y ?>"><?= $y ?></option><?php endforeach; ?></select>
    ...
    <div class="card-body table-responsive">
      <div class="d-flex justify-content-between align-items-center mb-3">
        ...
        <div class="d-flex gap-2">
          ...
          <select id="yearDropdown" class="form-select form-select-sm" style="height:40px;font-size:0.875rem;width:120px;background-color:#f1f5f9;border:1px solid #94a3b8;color:#1e293b;" data-edit-url="<?= $baseUrl ?>/modules/get_year_data.php?module=<?= $moduleKey ?>">
            <option value="">Select a Year</option>
            <?php foreach ($years as $year): ?><option value="<?= $year ?>"><?= $year ?></option><?php endforeach; ?>
          </select>
      ...
      <table class="table table-bordered align-middle">
        <thead class="table-light text-center">
          <tr><th>Key Result Areas</th><?php foreach ($years as $year): ?><th><?= htmlspecialchars($year) ?></th><?php endforeach; ?></tr>
        </thead>
        <tbody>
          ...
          <tr>
            ...
            <td><?php if (isset($data[$category][$year]) && !empty($data[$category][$year])): ?><ul class="mb-0"><?php foreach ($data[$category][$year] as $item): ?><li><?= htmlspecialchars($item) ?></li><?php endforeach; ?></ul><?php endif; ?></td>
            <?php endforeach; ?>
          </tr>
          <?php endforeach; ?>
        </tbody>
...
<div class="modal fade" id="addYearModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <form method="POST" action="<?= $baseUrl ?>/modules/add_year.php?module=<?= $moduleKey ?>" id="addYearForm">
...
<div class="modal fade" id="editYearModal" tabindex="-1">
  <div class="modal-dialog modal-lg">
    <form method="POST" action="<?= $baseUrl ?>/modules/edit_year.php?module=<?= $moduleKey ?>" id="editYearForm">
      <div class="modal-content">
...
<div class="modal fade" id="deleteYearModal" tabindex="-1">
  <div class="modal-dialog">
    <form method="POST" action="<?= $baseUrl ?>/modules/delete_year.php?module=<?= $moduleKey ?>" id="deleteYearForm">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-3"><label for="yearToDelete" class="form-label">Select Year to Delete</label>
            <select class="form-select" id="yearToDelete" name="year" required><option value="">-- Select Year --</option><?php foreach ($years as $year): ?><option value="<?= htmlspecialchars($year) ?>"><?= htmlspecialchars($year) ?></option><?php endforeach; ?></select>
          </div>
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          <div class="mb-3"><label for="rowToDelete" class="form-label">Select Key Area to Delete</label>
            <select class="form-select" id="rowToDelete" name="category" required><option value="">-- Select Key Area --</option><?php foreach ($categories as $cat): ?><option value="<?= htmlspecialchars($cat) ?>"><?= htmlspecialchars($cat) ?></option><?php endforeach; ?></select>
          </div>
...
<div class="modal fade" id="addRowModal" tabindex="-1">
  <div class="modal-dialog">
    <form class="modal-content" id="addRowForm">
      ...
      <div class="modal-body">
        <div class="mb-3"><label class="form-label">Category</label><select class="form-select" name="category" required><option value="">Select Category</option><?php foreach ($categories as $c): ?><option value="<?= htmlspecialchars($c) ?>"><?= htmlspecialchars($c) ?></option><?php endforeach; ?></select></div>
