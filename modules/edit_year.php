...
try {
  ...
  if (!$stmt->execute()) throw new Exception("Failed to delete existing data for year $year: " . $stmt->error);
  $stmt->close();
  ...
  foreach ($categories as $originalCategoryName => $categoryData) {
    ...
    foreach ($descriptions as $description) {
      ...
      if (!$stmt->execute()) throw new Exception("Insert failed for category '$categoryName': " . $stmt->error);
      $stmt->close();
    }
  }
  $conn->commit();
