...
$stmt = $conn->prepare("SELECT a.*, u.email AS uploader_email FROM cascading_activities a JOIN users u ON a.uploaded_by = u.id WHERE a.id = ?");
...
$filePath = __DIR__ . '/uploads/cascading_activities/' . $file['filename'];
...
<body>
  ...
  <div class="page-wrapper">
    ...
    <div class="header">
        ...
        <div>
            <a href="uploads/cascading_activities/<?= h($file['filename']) ?>" class="btn btn-light btn-sm me-2" download>
                <i data-lucide="download" class="me-1"></i> Download
            </a>
            <a href="cascading_activities.php" class="btn btn-outline-light btn-sm">
                <i data-lucide="arrow-left" class="me-1"></i> Back
            </a>
        </div>
    </div>
    <iframe src="uploads/cascading_activities/<?= h($file['filename']) ?>" class="pdf-viewer"></iframe>
      </div>
    ...
    <div class="container">
        <div class="header">
            <div class="d-flex justify-content-between align-items-center">
                ...
                <div>
                    <a href="uploads/cascading_activities/<?= h($file['filename']) ?>" class="btn btn-primary btn-sm me-2" download>
                        <i data-lucide="download" class="me-1"></i> Download
                    </a>
                    <a href="cascading_activities.php" class="btn btn-outline-secondary btn-sm">
                        <i data-lucide="arrow-left" class="me-1"></i> Back
                    </a>
                </div>
            </div>
        ...
        <div class="image-container">
            <img src="uploads/cascading_activities/<?= h($file['filename']) ?>" alt="<?= h($file['original_name']) ?>">
        </div>
    </div>
      </div>
  <?php include PGS_TEMPLATES . '/footer.php'; ?>
